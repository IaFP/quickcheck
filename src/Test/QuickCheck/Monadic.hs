...
#ifndef NO_MONADFAIL
instance (Total m, Monad m) => Fail.MonadFail (PropertyM m) where
  fail = fail_
#endif
...
instance (Total m, Monad m, MonadIO m) => MonadIO (PropertyM m) where
  liftIO = run . liftIO
#endif

stop :: (Total m, Testable prop, Monad m) => prop -> PropertyM m a
...
assert :: (Total m, Monad m) => Bool -> PropertyM m ()
assert True  = return ()
...
-- @
assertWith :: (Total m, Monad m) => Bool -> String -> PropertyM m ()
...
-- @
--
pre :: (Total m, Monad m) => Bool -> PropertyM m ()
pre True  = return ()
...
--   -- ...
-- @
run :: (Total m, Monad m) => m a -> PropertyM m a
...
pick :: (Total m, Monad m, Show a) => Gen a -> PropertyM m a
...
wp :: (Total m, Monad m) => m a -> (a -> PropertyM m b) -> PropertyM m b
wp m k = run m >>= k
...
forAllM :: (Total m, Monad m, Show a) => Gen a -> (a -> PropertyM m b) -> PropertyM m b
...
monitor :: (Total m, Monad m) => (Property -> Property) -> PropertyM m ()
